...
echo.
echo mode：
echo   d3d11       - ANGLE d3d11（默认，优先硬件加速 + WebGL2）
echo   d3d9        - ANGLE d3d9（更稳，但多数环境下仅 WebGL1）
...
if "%SW_CLEAN%"=="1" (
  if "%SW_VERBOSE%"=="1" echo [SpecWave] 清理残留进程（仅匹配当前仓库路径）...
  set "SW_ROOT=%CD%"
...
if "%SW_VERBOSE%"=="1" echo [SpecWave] 启动：ANGLE=%SPECWAVE_ANGLE% USE_GL=%SPECWAVE_USE_GL% DISABLE_GPU=%SPECWAVE_DISABLE_GPU%
